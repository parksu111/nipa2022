...
def load_result(path, pred=False, cols=None):
    label_encoding = {'other':0, 'noise':1, 'normal':2, 'in':3, 'out':4}
    try:
        ...
        for i,sample in enumerate(result['leaktype']):
            assert sample in label_encoding.keys(), f'invalid category: {sample}'
            result.loc[i, 'leaktype'] = label_encoding[sample]
        ...
        return list(result['id']), list(result['leaktype']), p_type_li, cols
